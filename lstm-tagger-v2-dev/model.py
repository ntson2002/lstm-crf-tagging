...
class Model(object):
    ...
    def save_mappings(self, id_to_word, id_to_char, id_to_tag, id_to_pos, id_to_chunk):
        """
    ...
    def build(self,
        ...
        print "SIZEs (model.py:133) words - chars - poss - chunks - tags: ", n_words, n_chars, n_poss, n_chunks, n_tags
        ...
        if pos_dim:
            input_dim += pos_dim
            pos_layer = EmbeddingLayer(n_poss, pos_dim, name='pos_layer')
            inputs.append(pos_layer.link(pos_ids))

        if chunk_dim:
            input_dim += chunk_dim
            chunk_layer = EmbeddingLayer(n_chunks, chunk_dim, name='chunk_layer')
            inputs.append(chunk_layer.link(chunk_ids))
